...
**Etapa atual: 3.1 concluída. Corrigindo bugs antes de avançar para 3.2.**

---

#### ✅ Etapa 3.0 — Base de Dados Real (CONCLUÍDA — 29/04/2026)
- [x] Banco zerado e recriado
- [x] JSONs mockados arquivados em `src/data/archive/`
- [x] 221 transações importadas

#### ✅ Etapa 3.1 — Nubank PF → Dashboard Real (CONCLUÍDA — 29/04/2026)
- [x] Banco zerado novamente
- [x] Extrato Nubank PF importado (71 transações, mar/2026)
- [x] `GET /api/dashboard/monthly` criado
- [x] `useFinancialData` refatorado para `fetch()`
- [x] Dashboard Anual com dados reais
- [x] Visão Mensal com dados reais
- [x] LoadingSpinner, ErrorState, EmptyState criados
- [x] Build de produção limpo

---

#### 🔴 Etapa 3.1.1 — Correções pós-validação (ATUAL)

Bugs e melhorias identificados após a primeira importação real:

**🐛 Bug 1 — Tela de Upload: falta botão "Cancelar"**
- Na tela de preview de lançamentos, após o upload processar o arquivo,
  existe o botão "Importar selecionados" mas não existe um botão "Cancelar"
- O usuário fica preso — precisa de uma saída para voltar sem importar
- Comportamento esperado: botão "Cancelar" ao lado do "Importar selecionados"
  → descarta o preview e volta para a drop zone de upload

**🐛 Bug 2 — Tabela de preview com scroll infinito**
- A tabela de lançamentos na tela de upload ocupa a página inteira
- Ao ter muitos itens (ex: 93 transações do Nubank PF), a página fica
  com scroll infinito e o botão de importar some para baixo
- Comportamento esperado:
  - Tabela com altura fixa (ex: max-h-[60vh] ou similar)
  - Scroll interno da tabela, não da página
  - Botões "Cancelar" e "Importar selecionados" sempre visíveis no rodapé,
    fora da tabela com scroll

**🐛 Bug 3 — Visão Mensal: detalhes de gastos zerados**
- O Dashboard mostra corretamente: total entradas, total gastos, saldo
- Mas na Visão Mensal (`/mes/2026-03`), as seções "Contas Fixas/Moradia"
  e "Fatura Cartão" aparecem zeradas, mesmo com transações importadas
- Causa provável: o endpoint `GET /api/dashboard/monthly` agrega os totais
  corretamente, mas as seções colapsáveis da Visão Mensal precisam dos
  lançamentos individuais (não só os totais) para listar cada gasto
- Comportamento esperado:
  - Seção "Entradas" lista cada transação de entrada individualmente
  - Seção "Contas Fixas/Moradia" lista cada débito individualmente
  - Seção "Fatura Cartão" lista as transações da conta nubank_cartao
  - Dados vêm de `GET /api/transactions?month=YYYY-MM&account=...`

**🔧 Melhoria 4 — Persistência de todas as extrações**
- Atualmente só é possível ver os dados da última importação
- Toda extração confirmada deve ser persistida no banco e sempre
  consultável, independente de quando foi importada
- Comportamento esperado (já deve funcionar via banco, validar):
  - `GET /api/transactions?month=2026-03` sempre retorna todas as
    transações de março, mesmo que a importação tenha sido feita dias atrás
  - Se não funcionar, é bug no endpoint de listagem

---

#### ⚫ Etapa 3.2 — Expandir para outros bancos (aguardando correções 3.1.1)
- [ ] Importar Nubank PJ
- [ ] Importar Itaú Uniclass
- [ ] Importar Mercado Pago
- [ ] Importar Fatura Cartão Nubank
- [ ] Dashboard consolidado com todos os bancos
...
### 🤖 IA / Categorização (Fase 4+)

- [ ] **Motor de categorização configurável** — regras determinísticas mapeando descrição → categoria (ex: "COPEL" → "Energia"). Descartado do MVP da Fase 3 para simplificar.
- [ ] Categorização inteligente com IA (Claude API)
...
- [ ] Tela `/categorias` para gerenciar regras

### 💼 Investimentos / B3 (Fase 4+)

- [ ] Parser B3 — Posição (`.xlsx`) — aguarda arquivos reais
- [ ] Parser B3 — Movimentação (`.xlsx`)
- [ ] Parser B3 — Negociação (`.xlsx`)
- [ ] Tela Carteira B3 com dados reais
- [ ] Tela Proventos com dados reais
...
- [ ] Cálculo automático de IR sobre vendas
