...
export default function CategoriesPage() {
  ...
  const filteredParents = useMemo(() => {
    ...
    return parents.filter((cat) => {
      ...
      const subHit = (subsByParent.get(cat.id) ?? []).some((s) =>
    ...
    setForm({
      ...
      budgetLimit:
        category.invoice_budget_limit != null
          ? String(category.invoice_budget_limit)
          : "",
    });
    setFormError(null);
      ...
      if (modal.mode === "edit") {
        ...
        const payload = {
          ...
          invoice_budget_limit:
            form.scope === "credit_card" ? (limit ?? null) : undefined,
        };
  ...
  async function handleDelete(category: Category) {
    ...
    try {
      ...
      setCategories((prev) =>
        prev.filter((c) => c.id !== category.id && c.parent_id !== category.id),
      );
    } catch (err) {
      setError(
        ...
        >
          ...
          >
            ...
            {domain === "credit_card" ? (
              <>
                <SummaryCard
                  label="Com limite definido"
                  value={String(summary.withLimit)}
                />
                <SummaryCard
                  label="Sem limite"
                  value={String(summary.withoutLimit)}
                />
                <SummaryCard
                  label="Aplicadas a todos"
                  value={String(summary.globals)}
                />
              </>
            ) : (
          ...
          >
            {categories.length === 0 ? (
              ...
              <>
                ...
                <div style={{ fontSize: 13 }}>
                  Você tem <strong>{categories.length}</strong> categoria
                  {categories.length === 1 ? "" : "s"} cadastrada
                  {categories.length === 1 ? "" : "s"}, mas nenhuma categoria
                  principal foi encontrada para listar.
                </div>
                <div
                  style={{
                    fontSize: 12,
                ...
                >
                  Isso pode indicar um problema de dados. Tente recarregar; se
...
export default function CategoriesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Categorias"
        subtitle="Organize e defina limites de orçamento"
      />
