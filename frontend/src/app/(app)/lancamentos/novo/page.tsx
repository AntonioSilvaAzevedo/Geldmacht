...
import { ArrowLeft, CreditCard, FileUp, Landmark, Loader2 } from 'lucide-react';
...
export default function LancamentoNovoPage() {
  ...
  if (step === 'choose') {
    return (
      <>
        <PageHeader title="Adicionar lançamento" subtitle="Escolha como deseja registrar." />
        <main style={{ padding: isMobile ? 16 : '28px 32px', flex: 1, maxWidth: 560, margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'grid', gap: 12 }}>
            <button
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--blue-500)'; }}
  ...
  return (
    <>
      <PageHeader title="Lançamento manual" subtitle="Conta bancária — entradas e saídas." />
      <main style={{ padding: isMobile ? 16 : '24px 32px', flex: 1, maxWidth: 520, margin: '0 auto', width: '100%' }}>
        ...
        {loadingRefs ? (
          ...
          <div style={{
            ...
            <Link href="/contas" style={{ ...primaryBtn, textDecoration: 'none', display: 'inline-flex' }}>
          ...
          <form onSubmit={e => void handleSubmit(e)} style={{ display: 'grid', gap: 14 }}>
            ...
            <label style={{ display: 'grid', gap: 5, fontSize: 12 }}>
              ...
              <input
                ...
                inputMode="decimal"
                placeholder="0,00"
                style={{ padding: '9px 11px', borderRadius: 8, border: '1px solid var(--border-default)', background: 'var(--surface-card)', color: 'var(--text-primary)', fontSize: 14, fontFamily: 'var(--font-mono)' }}
              />
            </label>
            ...
            <div style={{ display: 'grid', gap: 5, fontSize: 12 }}>
              ...
              {categories.length === 0 ? (
                <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)' }}>
                  ...
                  <Link href="/categorias" style={{ color: 'var(--blue-400)' }}>Criar em Categorias</Link>
                  {' '}(aba Conta bancária).
                </p>
              ) : (
            ...
            <label style={{ display: 'grid', gap: 5, fontSize: 12 }}>
              ...
              <textarea
                ...
                rows={2}
                style={{ padding: '9px 11px', borderRadius: 8, border: '1px solid var(--border-default)', background: 'var(--surface-card)', color: 'var(--text-primary)', fontSize: 13, resize: 'vertical' }}
              />
            </label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
              <button type="submit" disabled={saving} style={{ ...primaryBtn, flex: 1, opacity: saving ? 0.75 : 1 }}>
                {saving ? <><Loader2 size={16} /> Salvando…</> : 'Salvar lançamento'}
              </button>
            </div>
          </form>
        )}
