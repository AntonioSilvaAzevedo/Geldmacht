...
function InvoiceNavBar({
  ...
  return (
    <div style={{
      ...
      <button onClick={onPrev} disabled={!prev} style={{
        ...btnBase,
        borderColor: prev ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.04)',
        color: prev ? 'var(--text-primary)' : 'rgba(255,255,255,0.18)',
      ...
      <button onClick={onNext} disabled={!next} style={{
        ...btnBase,
        borderColor: next ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.04)',
        color: next ? 'var(--text-primary)' : 'rgba(255,255,255,0.18)',
                    ...
                    {group.transactions.map((tx, i) => (
                      <div key={tx.id} style={{
                        ...
                        <div style={{ textAlign:'right', flexShrink:0 }}>
                          ...
                          <button onClick={e => { e.stopPropagation(); onRecategorize(tx.id); }} style={{
                            background:'none', border:'none', color:'var(--blue-400)',
                            fontSize:10, cursor:'pointer', fontFamily:'var(--font-sans)', marginTop:2,
                          }}>
                            Categoria
                          </button>
                        </div>
        ...
        <div role="dialog" aria-modal="true" onClick={e => { if(e.target===e.currentTarget && !recatSaving) setRecatModal(null); }}
          ...
          <div onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:400, background:'var(--surface-card)', borderRadius:'var(--radius-lg)', border:'1px solid var(--border-subtle)', padding:'var(--space-5)', boxShadow:'var(--shadow-modal)' }}>
            ...
            <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:22 }}>
              <button disabled={recatSaving} onClick={()=>setRecatModal(null)} style={ghostBtnStyle}>Cancelar</button>
              <button disabled={recatSaving} onClick={()=>void saveRecat()} style={primaryBtnStyle}>
...
const primaryBtnStyle: React.CSSProperties = { padding:'9px 14px', borderRadius:'var(--radius-md)', border:'none', background:'var(--blue-500,var(--blue-400))', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer' };
const ghostBtnStyle:   React.CSSProperties = { padding:'9px 14px', borderRadius:'var(--radius-md)', border:'1px solid var(--border-default)', background:'transparent', color:'var(--text-secondary)', fontSize:13, cursor:'pointer' };
